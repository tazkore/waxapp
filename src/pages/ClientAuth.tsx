...
import { User, Mail, Eye, EyeOff, Lock, Phone, Globe, CheckCircle, Sparkles, Apple } from 'lucide-react';
...
const ClientAuth = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      toast({ title: 'Error', description: error.message === 'Email not confirmed' ? 'Debes verificar tu correo electrónico antes de iniciar sesión.' : 'Credenciales inválidas.', variant: 'destructive' });
    } else {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (data.user) {
      const { error: profileError } = await supabase.from('customer_profiles').insert({
        ...
        full_name: fullName, phone, country,
        address: address || null, city: city || null, state: state || null, postal_code: postalCode || null,
      });
      if (profileError) {
      ...
      <Card className="w-full max-w-md border-border/50 bg-card">
        <CardHeader className="text-center space-y-2">
          ...
          <CardDescription className="text-muted-foreground flex items-center justify-center gap-1.5">
            <Sparkles className="w-3.5 h-3.5 text-primary" /> Elige tu forma de entrar
          </CardDescription>
        </CardHeader>
        <CardContent>
          {signupDone ? (
            ...
            <>
              <Tabs defaultValue="password" className="w-full">
                <TabsList className="grid w-full grid-cols-3 bg-muted">
                  <TabsTrigger value="password" className="data-[state=active]:border data-[state=active]:border-primary/60 data-[state=active]:text-primary">Contraseña</TabsTrigger>
                  <TabsTrigger value="magic" className="data-[state=active]:border data-[state=active]:border-primary/60 data-[state=active]:text-primary">Magic Link</TabsTrigger>
                  <TabsTrigger value="social" className="data-[state=active]:border data-[state=active]:border-primary/60 data-[state=active]:text-primary">Social</TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="magic" className="space-y-4 pt-4">
                  {magicSent ? (
                    <div className="text-center space-y-3 py-4">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Te enviamos un enlace mágico a <strong className="text-foreground">{magicEmail}</strong>. Haz clic para entrar sin contraseña.
                      </p>
                      <Button variant="outline" onClick={() => setMagicSent(false)} className="mt-2">Enviar a otro correo</Button>
                    </div>
                  ) : (
                    <form onSubmit={handleMagicLink} className="space-y-4">
                      <p className="text-sm text-muted-foreground">Sin contraseñas. Te enviamos un enlace seguro a tu correo.</p>
                      <div className="space-y-2">
                        <Label htmlFor="magicEmail" className="text-foreground">Email</Label>
                        <div className="relative">
                          <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                          <Input id="magicEmail" type="email" placeholder="tu@email.com" value={magicEmail} onChange={(e) => setMagicEmail(e.target.value)} className="pl-10 bg-muted border-border" required />
                        </div>
                      </div>
                      <Button type="submit" className="w-full" disabled={magicLoading}>
                        {magicLoading ? 'Enviando...' : 'Enviarme el Magic Link'}
                      </Button>
                    </form>
                  )}
                </TabsContent>
                ...
                <TabsContent value="social" className="space-y-3 pt-4">
                  <p className="text-sm text-muted-foreground text-center">Entra con tu cuenta favorita.</p>
                  <Button type="button" variant="outline" className="w-full gap-2 border-border" onClick={() => handleOAuth('google')}>
                  ...
                  <Button type="button" variant="outline" className="w-full gap-2 border-border" onClick={() => handleOAuth('apple')}>
                    <Apple className="h-4 w-4" />
                    Continuar con Apple
                  </Button>
                  <p className="text-[11px] text-center text-muted-foreground pt-2">
                    GitHub no está disponible por ahora — usa email o Google.
                  </p>
                </TabsContent>
              </Tabs>
